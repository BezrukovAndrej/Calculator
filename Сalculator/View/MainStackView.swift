...
final class MainStackView: UIStackView {
    ...
    private func setupMainStackView() {
        ...
        let buttoEqual = createButton(title: "=", tag: 11, color: .orange)
        let firstStackView = UIStackView(subview: [button0, buttonPoint, buttoEqual])
        button0.widthAnchor.constraint(equalTo: firstStackView.widthAnchor, multiplier: 0.5).isActive = true
        buttonPoint.widthAnchor.constraint(equalTo: buttoEqual.widthAnchor, multiplier: 1).isActive = true
        ...
        let secondStackView = UIStackView(subview: [button1, button2, button3, buttonPlus])
        ...
        let thirdStackView = UIStackView(subview: [button4, button5, button6, buttonMinus])
        ...
        let fourthStackView = UIStackView(subview: [button7, button8, button9, buttonMultiply])
        
        let buttonAC = createButton(title: "AC", tag: 18, color: .darkGray)
        let buttonPlusMinus = createButton(title: "+/-", tag: 17, color: .darkGray)
        let buttonProcent = createButton(title: "%", tag: 16, color: .darkGray)
        let buttonDivision = createButton(title: "/", tag: 15, color: .orange)
        let fifthStackView = UIStackView(subview: [buttonAC, buttonPlusMinus, buttonProcent, buttonDivision])
